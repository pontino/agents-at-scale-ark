...
- **PgBouncer in transaction or statement pooling mode is incompatible with logical replication.** The replication connection must reach Postgres directly (or through a session-pooling/`session` mode pooler). Point `ARK_POSTGRES_HOST` at a direct endpoint, or run a separate session-mode pooler for the apiserver.
